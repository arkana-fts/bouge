...
namespace bouge
{
    ...
    bool CoreMaterial::hasProperty(const std::string& name) const
    {
        return m_properties.find(name) != m_properties.end();
    }

    const std::string& CoreMaterial::property(const std::string& name) const
    {
        static std::string emptystr;
        PropertyMap::const_iterator i = m_properties.find(name);
        return i != m_properties.end() ? i->second.val() : emptystr;
    }

    const std::vector<float>& CoreMaterial::propertyAsFvec(const std::string& name) const
    {
        PropertyMap::const_iterator i = m_properties.find(name);
    ...
    CoreMaterial& CoreMaterial::property(const std::string& name, const std::string value)
    {
        m_properties[name].val(value);
        return *this;
    }

    CoreMaterial& CoreMaterial::property(const std::string& name, const std::vector<float> value)
    {
        m_properties[name].val(to_s(value));
        return *this;
    }

    CoreMaterial::iterator::iterator(PropertyMap::iterator me)
        : myIter(me)
    { }
    ...
    {
        return iterator(m_properties.begin());
    }

    CoreMaterial::iterator CoreMaterial::end()
    {
        return iterator(m_properties.end());
    }

    CoreMaterial::const_iterator::const_iterator(PropertyMap::const_iterator me)
        : myIter(me)
    { }
